...
def apply_oom_policy(spec: JobSpec, ctx: OomContext) -> tuple[JobSpec | None, str]:
    """Apply policy steps in order; return (new_spec or None, action name)."""
    for step in parse_policy(spec.on_oom):
        if isinstance(step, BumpMemStep) and ctx.class_ == "cpu":
            ...
            if new_mb <= current_mb:
                continue
            return spec.model_copy(update={"mem": f"{new_mb}M"}), "bump_mem"
        if isinstance(step, EscalatePartitionStep):
            ...
            if spec.partition is not None:
                return spec.model_copy(update={"partition": None}), "escalate_partition"
            continue
